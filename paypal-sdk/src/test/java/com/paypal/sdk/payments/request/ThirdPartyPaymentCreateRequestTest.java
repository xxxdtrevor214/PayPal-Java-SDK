...
public class ThirdPartyPaymentCreateRequestTest extends TestHarness {

	public static HttpResponse<Payment> createThirdPartyPayment(HttpClient client, PayPalEnvironment environment, String intent) throws IOException {
		return createThirdPartyPayment(client, environment, "credit_card", intent);
	}

	public static HttpResponse<Payment> createThirdPartyPayment(HttpClient client, PayPalEnvironment environment, String paymentMethod, String intent) throws IOException {

		RefreshTokenRequest refreshTokenRequest = new RefreshTokenRequest(environment, "1312312312312");
		HttpResponse<RefreshToken> response = client.execute(refreshTokenRequest);

		String refreshToken = response.result().refreshToken();
		System.out.println("Refresh token: " + refreshToken);

		AccessTokenRequest accessTokenRequest = new AccessTokenRequest(environment, refreshToken);
		HttpResponse<AccessToken> accessTokenHttpResponse = client.execute(accessTokenRequest);

		String accessToken = accessTokenHttpResponse.result().accessToken();
		System.out.println("Access token: " + accessToken);

		ThirdPartyPaymentCreateRequest request = new ThirdPartyPaymentCreateRequest(accessToken);
	...
	public void testPaymentCreateRequest() throws IOException {
		HttpResponse<Payment> response = ThirdPartyPaymentCreateRequestTest.createThirdPartyPayment(client(), environment(), "sale");
