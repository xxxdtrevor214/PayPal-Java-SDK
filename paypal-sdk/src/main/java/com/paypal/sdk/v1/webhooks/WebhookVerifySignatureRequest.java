// This class was generated on Mon, 29 Jan 2018 12:12:04 PST by version 0.1.0-dev+ecea6f of Braintree SDK Generator
// WebhookVerifySignatureRequest.java
// @version 0.1.0-dev+ecea6f
// @type request
// @data 
// DO NOT EDIT
package com.paypal.sdk.v1.webhooks;

import com.braintreepayments.http.*;
import java.util.*;
import java.util.stream.Collectors;
import java.io.UnsupportedEncodingException;
import java.net.URLEncoder;
/**
 * Verifies a webhook signature.
 */
public class WebhookVerifySignatureRequest extends HttpRequest<VerifyWebhookSignatureResponse> {

    public WebhookVerifySignatureRequest() {
        super("/v1/notifications/verify-webhook-signature?", "POST", VerifyWebhookSignatureResponse.class);
        
        header("Content-Type", "application/json");
    }
    

    public WebhookVerifySignatureRequest requestBody(VerifyWebhookSignature verifyWebhookSignature) {
        super.requestBody(verifyWebhookSignature);
        return this;
    }
}
