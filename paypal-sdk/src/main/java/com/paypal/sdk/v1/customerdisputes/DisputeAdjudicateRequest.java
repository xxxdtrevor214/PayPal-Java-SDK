// This class was generated on Mon, 09 Apr 2018 16:10:52 UTC by version 0.1.0-dev+291f3f of Braintree SDK Generator
// DisputeAdjudicateRequest.java
// @version 0.1.0-dev+291f3f
// @type request
// @data 
// DO NOT EDIT
package com.paypal.sdk.v1.customerdisputes;

import com.braintreepayments.http.*;
import java.util.*;
import java.util.stream.Collectors;
import java.io.UnsupportedEncodingException;
import java.net.URLEncoder;
/**
 * Settles a dispute, by ID, in either the buyer's or seller's favor.
 */
public class DisputeAdjudicateRequest extends HttpRequest<SubsequentActions> {

    public DisputeAdjudicateRequest(String disputeId) {
        super("/v1/customer/disputes/{dispute_id}/adjudicate?", "POST", SubsequentActions.class);
        try {
            path(path().replace("{dispute_id}", URLEncoder.encode(String.valueOf(disputeId), "UTF-8")));
        } catch (UnsupportedEncodingException ignored) {}
        
        header("Content-Type", "application/json");
    }
    

    public DisputeAdjudicateRequest requestBody(AdjudicateRequest body) {
        super.requestBody(body);
        return this;
    }
}
