// This class was generated on Fri, 26 Jan 2018 15:53:38 PST by version 0.1.0-dev+30efd2 of Braintree SDK Generator
// UserinfoGetRequest.java
// @version 0.1.0-dev+30efd2
// @type request
// @data 
// DO NOT EDIT
package com.paypal.sdk.identity;

import com.braintreepayments.http.*;
import java.util.*;
import java.util.stream.Collectors;
import java.io.UnsupportedEncodingException;
import java.net.URLEncoder;
/**
 * Shows user details.<blockquote><strong>Note:</strong> This API is deprecated. Use `v1/oauth2/token/userinfo` instead.</blockquote>
 */
public class UserinfoGetRequest extends HttpRequest<UserInfo> {

    public UserinfoGetRequest() {
        super("/v1/identity/openidconnect/userinfo?", "GET", UserInfo.class);
        
    }

    public UserinfoGetRequest schema(String schema) {
        String params = String.valueOf(schema);
        try {
		    path(path() + "schema=" + URLEncoder.encode(params, "UTF-8") + "&");
        } catch (UnsupportedEncodingException ignored) {}
        return this;
    }
    
}
