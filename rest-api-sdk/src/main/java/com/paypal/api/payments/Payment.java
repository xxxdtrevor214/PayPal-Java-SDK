...
public class Payment extends Resource {
	 ...
	 * @param containerMap
	 *            Map containing the query strings with the following values as
	 *            keys: count, start_id, start_index, start_time, end_time,
	 *            payee_id, sort_by, sort_order, All other keys in the map are
	 *            ignored by the SDK
	 * @HttpMethod GET
	 * @URIpath 
	 *          v1/payments/payment?count=:count&start_id=:start_id&start_index=:
	 *          start_index
	 *          &start_time=:start_time&end_time=:end_time&payee_id=:payee_id
	...
	public static PaymentHistory get(String accessToken,
		...
		return PayPalResource.configureAndExecute(apiContext, HttpMethod.GET,
				resourcePath, payLoad, PaymentHistory.class);
	}
	 ...
	 * @return PaymentHistory
	 */
	public static PaymentHistory get(APIContext apiContext,
			Map<String, String> containerMap) throws PayPalRESTException {
		String pattern = "v1/payments/payment?count={0}&start_id={1}&start_index={2}&start_time={3}&end_time={4}&payee_id={5}&sort_by={6}&sort_order={7}";
		Object[] parameters = new Object[] { containerMap };
		String resourcePath = RESTUtil.formatURIPath(pattern, parameters);
		...
		if (apiContext.getAccessToken() == null
			...
			throw new IllegalArgumentException(
					"AccessToken cannot be null in APIContext");
		}
		return PayPalResource.configureAndExecute(apiContext, HttpMethod.GET,
				resourcePath, payLoad, PaymentHistory.class);
	}
	 ...
	 */
	public static PaymentHistory get(String accessToken,
			QueryParameters queryParameters) throws PayPalRESTException {
		String pattern = "v1/payments/payment?count={0}&start_id={1}&start_index={2}&start_time={3}&end_time={4}&payee_id={5}&sort_by={6}&sort_order={7}";
	...
	public Payment create(APIContext apiContext) throws PayPalRESTException {
		...
		return PayPalResource.configureAndExecute(apiContext, HttpMethod.POST,
				resourcePath, payLoad, Payment.class);
	}
	...
	public Payment execute(APIContext apiContext,
			PaymentExecution paymentExecution) throws PayPalRESTException {
		if (paymentExecution == null) {
			throw new IllegalArgumentException(
					"paymentExecution cannot be null");
		}
		if (this.getId() == null) {
